...
@section('content')
  ...
  <form method="POST" action="{{ route('admin.pages.update', $page) }}">
    ...
    <div class="form-group">
      ...
      <input id="menu_title" class="form-control{{ $errors->has('menu_title') ? ' is-invalid' : '' }}" name="menu_title"
    ...
    <div class="form-group">
      ...
      <textarea id="content" class="form-control{{ $errors->has('content') ? ' is-invalid' : '' }} summernote"
        data-image-url="{{ route('admin.ajax.upload.image') }}" name="content" rows="10" required>{{ old('content', $page->content) }}</textarea>
